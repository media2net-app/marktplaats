...
async def choose_category(page: Page, category_path: Optional[str]) -> None:
	"""Select a category by following the category path (e.g., 'Wonen > Slaapkamer > Bedden')"""
	if not category_path:
		return
	
	print(f"  Choosing category path: {category_path}")
	parts = [p.strip() for p in category_path.split('>') if p.strip()]
	
	# First, try to click "Of selecteer zelf een categorie" if available
	try:
	...
	# Navigate through each part of the category path
	for idx, part in enumerate(parts):
		print(f"    Selecting: {part}")
		...
		# Try with data-testid or other attributes
		if not locator or await locator.count() == 0:
			try:
				# Try to find in category list items
				locator = page.locator(f"text={part}").first
				if await locator.count() == 0:
					locator = None
			except Exception:
				pass
		...
		if locator and await locator.count() > 0:
			try:
				...
				await page.wait_for_timeout(500)  # Wait for category to load
			except Exception as e:
				print(f"    ⚠️  Could not click on '{part}': {e}")
				# Continue anyway, might still work
		else:
			print(f"    ⚠️  Category part '{part}' not found, skipping...")
			# If we can't find a category part, break to avoid errors
			break
...
async def upload_photos(page: Page, product: Product, media_root: str) -> None:
	...
	if page.is_closed():
		print("Page is closed, skipping photo upload")
		return
	...
	try:
		...
		if page.is_closed():
			print("Page closed before photo upload")
			return
		...
		for sel in selectors:
			if page.is_closed():
				print("Page closed during file input search")
				return
			try:
		...
		if "Target page" in error_msg or "closed" in error_msg.lower():
			print(f"Photo upload error: Browser/page was closed during upload")
		else:
			print(f"Photo upload error: {e}")
		# Don't re-raise, continue with the process
...
async def run(csv_path: Optional[str], api_url: Optional[str], product_id: Optional[str], login_only: bool, keep_open: bool=False) -> Optional[List[Dict]]:
	...
	async with async_playwright() as p:
		...
		for index, product in enumerate(products, start=1):
			...
			# Use the category that was set for the product
			if product.category_path:
				print(f"  Using product category: {product.category_path}")
				# Skip auto-suggest and directly select the category
				await choose_category(page, product.category_path)
			else:
				# Only use auto-suggest if no category is set
				print(f"  No category set, using auto-suggest")
