    ...
    return (
      <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-8">
        <div className="flex justify-between items-center mb-6">
          <div>
            <h3 className="text-xl font-bold text-gray-900">Advertentie Statistieken</h3>
            <p className="text-sm text-gray-600 mt-1">Nog geen advertenties geplaatst</p>
          </div>
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
      <div className="bg-gradient-to-r from-gray-50 to-gray-100 px-6 py-5 border-b border-gray-200 flex justify-between items-center">
        <div>
          <h3 className="text-xl font-bold text-gray-900">Advertentie Statistieken</h3>
          <p className="text-sm text-gray-600 mt-1">{postedProducts.length} geplaatste advertentie{postedProducts.length !== 1 ? 's' : ''}</p>
        </div>
        <div className="flex items-center gap-3">
          {syncMessage && (
            <span className={`text-sm font-medium ${syncMessage.includes('✅') ? 'text-green-600' : 'text-red-600'}`}>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {postedProducts.map((product) => (
              <tr key={product.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="text-sm text-gray-900 font-mono">{product.marktplaatsAdId || '-'}</div>
                </td>
